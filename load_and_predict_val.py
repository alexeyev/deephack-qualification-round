...
try:
    ...
    (xt_r, xt_c, yt) = data_helpers.load_val_data()
    ...
    for x_test_r, x_test_c in data_helpers.test_data_generator(xt_r, xt_c,
                                                               ...
                                                               batch_size=10000):
        print(prefix + " " + str(x_test_r.shape) + " " + str(len(predictions)))
        predictions.extend(list(model.predict([x_test_r, x_test_c])[:, 0]))

    with open(prefix + "_val.txt", "w+") as w:
        w.write(str(predictions))

    df = pd.DataFrame({"id": list(range(len(predictions))), "human-generated": predictions})
    print(df.head())
    df.to_csv(prefix + "_validation-2.csv", index=None, columns=["id", "human-generated"])
    del df

except Exception as ex:
    print(prefix + " " + "Can't predict on valset")
    print(str(ex))
